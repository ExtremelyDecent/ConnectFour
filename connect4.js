...
function placeInTable(y, x) {
  ...
  const placeColumn = document.getElementById(`${y}-${x}`);
...
function checkForWin() {
  ...
  for (let y = 0; y < HEIGHT; y++) {
    for (let x = 0; x < WIDTH; x++) {
      let horiz = [[y, x], [y, x + 1], [y, x + 2], [y, x + 3]]; //horizontal
      let vert = [[y, x], [y + 1, x], [y + 2, x], [y + 3, x]]; //vertical
      let diagDR = [[y, x], [y + 1, x + 1], [y + 2, x + 2], [y + 3, x + 3]]; //Right diagonal check
      let diagDL = [[y, x], [y + 1, x - 1], [y + 2, x - 2], [y + 3, x - 3]]; //Left diagonal check
